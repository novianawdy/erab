...
@section('content')
    @include('users.partials.header', ['title' => __('Edit Client')])
                        ...
                        <form method="post" action="{{ route('clients.update', $client) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('address') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea name="address"  id="input-address" class="form-control form-control-alternative{{ $errors->has('email') ? ' is-invalid' : '' }}" rows="3" placeholder="{{ __('Address') }}" required>{{ old('address', $client->address) }}</textarea>
                                ...
                                <div class="form-group{{ $errors->has('contact') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="contact" id="input-contact" class="form-control form-control-alternative{{ $errors->has('contact') ? ' is-invalid' : '' }}" placeholder="{{ __('Contact') }}" value="{{ old('name', $client->contact) }}" required autofocus>
