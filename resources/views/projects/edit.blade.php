                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('project.print', $project) }}" class="btn btn-sm btn-danger">{{ __('Print') }}</a>
                        ...
                        <form method="post" action="{{ route('projects.update', $project) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" value="{{ $project->name }}" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old("name") }}" required autofocus>
                            ...
                            <div class="pl-lg-4">
                              <div class="job-detail-container">
                                @if (@count($project->jobs) > 0)
                                  @foreach ($project->jobs as $project_job)
                                    <div class="row job-detail-root">
                                      <div class="col-md-4">
                                        <div class="form-group{{ $errors->has('job_id') ? ' has-danger' : '' }}">
                                          ...
                                          <select name="job_id[]" class="select-job form-control form-control-alternative{{ $errors->has('job_id') ? ' is-invalid' : '' }}" placeholder="{{ __('Job') }}" required autofocus id="input-job_id">
                                            ...
                                            @foreach ($jobs as $job)
                                              <option value="{{ $job->id }}" {{ (collect(old('job_id', $project_job->project_details->job_id))->contains($job->id)) ? 'selected':'' }}>{{ $job->name }}</option>
                                            @endforeach
                                          </select>
                                      ...
                                      <div class="col-md-2">
                                        <div class="form-group{{ $errors->has('volume') ? ' has-danger' : '' }}">
                                          ...
                                          <input type="text" value="{{ $project_job->project_details->volume }}" name="volume[]" class="input-volume form-control form-control-alternative{{ $errors->has('volume') ? ' is-invalid' : '' }}" placeholder="{{ __('Volume') }}" required autofocus>
                                      ...
                                      <div class="col-md-3">
                                        <div class="form-group{{ $errors->has('subtotal_job') ? ' has-danger' : '' }}">
                                          ...
                                          <input type="number" value="{{ $project_job->project_details->subtotal_job }}" readonly name="subtotal_job[]" class="input-subtotal_job form-control form-control-alternative{{ $errors->has('subtotal_job') ? ' is-invalid' : '' }}" placeholder="{{ __('Subtotal Job') }}" required autofocus>
                                      ...
                                      <div class="col-md-3">
                                        <div class="form-group">
                                          ...
                                          <input readonly type="text" value="{{ $project_job->project_details->subtotal }}" name="subtotal[]" id="input-subtotal" class="form-control form-control-alternative" placeholder="{{ __('Subtotal') }}" autofocus>
                                        </div>
                                      </div>
                                    </div>
                                  @endforeach
